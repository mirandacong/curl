...
static int setup_initial_crypto_context(struct connectdata *conn)
{
  ...
  pnlen = Curl_qc_derive_pkt_num_protection_key(pn, sizeof(pn),
                                                secret, sizeof(secret),
                                                &conn->quic.hs_crypto_ctx);
  if(pnlen < 0) {
    return -1;
  ...
  ngtcp2_conn_set_initial_tx_keys(conn->quic.conn, key, keylen, iv, ivlen,
                                  pn, pnlen);
  ...
  ngtcp2_conn_set_initial_rx_keys(conn->quic.conn,
                                  key, keylen, iv, ivlen, pn, pnlen);
...
static int cb_extend_max_stream_id(ngtcp2_conn *tconn, uint64_t max_stream_id,
                                   void *user_data)
{
  struct connectdata *conn = (struct connectdata *)user_data;
  /* not clear what this wants */
  (void)max_stream_id;
  (void)tconn;
  (void)conn;
...
static void quic_callbacks(ngtcp2_conn_callbacks *c)
{
  ...
  c->extend_max_stream_id = cb_extend_max_stream_id;
}
